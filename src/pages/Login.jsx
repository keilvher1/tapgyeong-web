import { useState } from 'react'
import { useNavigate } from 'react-router-dom'
import { supabase, DEMO_USER_ID } from '../lib/supabase'

const KAKAO_JS_KEY = '0192783ba4afa37564a9e3ec7595b220'
const KAKAO_REST_KEY = '39b318ede845f101187c8b3f9d33355c'

export default function Login() {
  const navigate = useNavigate()
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState(null)

  // Kakao OAuth redirect login
  const handleKakaoLogin = () => {
    setIsLoading(true)
    const redirectUri = `${window.location.origin}/auth/kakao/callback`
    const kakaoAuthUrl = `https://kauth.kakao.com/oauth/authorize?client_id=${KAKAO_REST_KEY}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code`
    window.location.href = kakaoAuthUrl
  }

  // Demo login (for prototype)
  const handleDemoLogin = async () => {
    setIsLoading(true)
    try {
      const { data: user } = await supabase
        .from('tg_users')
        .select('*')
        .eq('id', DEMO_USER_ID)
        .single()

      if (user) {
        localStorage.setItem('tg_user', JSON.stringify(user))
        localStorage.setItem('tg_logged_in', 'true')
        navigate('/explore')
      }
    } catch (err) {
      setError('로그인 중 오류가 발생했습니다.')
      console.error(err)
    } finally {
      setIsLoading(false)
    }
  }

  return (
    <div style={styles.container}>
      {/* Background */}
      <div style={styles.bgTop} />

      {/* Logo Area */}
      <div style={styles.logoArea}>
        <div style={styles.logoIcon}>
          <i className="fa-solid fa-location-dot" style={{ fontSize: 32, color: '#fff' }} />
        </div>
        <h1 style={styles.appName}>TapGyeong</h1>
        <p style={styles.appNameKo}>탭경</p>
        <p style={styles.tagline}>
          NFC 스마트카드 하나로<br />
          경북 전역을 여행하다
        </p>
      </div>

      {/* Login Card */}
      <div style={styles.loginCard}>
        <h2 style={styles.loginTitle}>로그인</h2>
        <p style={styles.loginDesc}>소셜 계정으로 간편하게 시작하세요</p>

        {error && (
          <div style={styles.errorBox}>
            <i className="fa-solid fa-circle-exclamation" style={{ fontSize: 12, marginRight: 6 }} />
            {error}
          </div>
        )}

        {/* Kakao Login Button */}
        <button
          style={styles.kakaoBtn}
          onClick={handleKakaoLogin}
          disabled={isLoading}
        >
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" style={{ marginRight: 8 }}>
            <path d="M12 3C6.48 3 2 6.48 2 10.5c0 2.64 1.74 4.96 4.36 6.3-.14.52-.9 3.36-.93 3.58 0 0-.02.16.08.22.1.06.22.02.22.02.29-.04 3.38-2.22 3.92-2.6.76.1 1.56.16 2.36.16 5.52 0 10-3.48 10-7.68C22 6.48 17.52 3 12 3z" fill="#3C1E1E"/>
          </svg>
          카카오 로그인
        </button>

        {/* Divider */}
        <div style={styles.divider}>
          <div style={styles.dividerLine} />
          <span style={styles.dividerText}>또는</span>
          <div style={styles.dividerLine} />
        </div>

        {/* Demo Login */}
        <button
          style={styles.demoBtn}
          onClick={handleDemoLogin}
          disabled={isLoading}
        >
          <i className="fa-solid fa-user-astronaut" style={{ fontSize: 16, marginRight: 8 }} />
          {isLoading ? '로그인 중...' : '데모 체험하기'}
        </button>

        <p style={styles.demoHint}>
          <i className="fa-solid fa-info-circle" style={{ fontSize: 11, marginRight: 4 }} />
          데모 계정으로 모든 기능을 체험할 수 있어요
        </p>
      </div>

      {/* Features Preview */}
      <div style={styles.featuresRow}>
        {[
          { icon: 'fa-hand-pointer', label: 'NFC 태깅' },
          { icon: 'fa-wand-magic-sparkles', label: 'AI 추천' },
          { icon: 'fa-map', label: '맵 컬러링' },
          { icon: 'fa-ticket', label: '쿠폰' },
        ].map((f, i) => (
          <div key={i} style={styles.featureChip}>
            <i className={`fa-solid ${f.icon}`} style={{ fontSize: 14, color: '#4A6CF7' }} />
            <span style={{ fontSize: 10, color: '#8888A8', marginTop: 4 }}>{f.label}</span>
          </div>
        ))}
      </div>

      {/* Footer */}
      <div style={styles.footer}>
        <p style={{ fontSize: 11, color: '#A0A0B8' }}>
          2026 관광데이터 활용 공모전 · 팀 탐경
        </p>
      </div>
    </div>
  )
}

const styles = {
  container: {
    minHeight: '100vh',
    background: '#F5F7FB',
    display: 'flex',
    flexDirection: 'column',
    alignItems: 'center',
    position: 'relative',
    overflow: 'hidden',
  },
  bgTop: {
    position: 'absolute',
    top: 0,
    left: 0,
    right: 0,
    height: 320,
    background: 'linear-gradient(160deg, #1E3A8A 0%, #4A6CF7 40%, #7DD3FC 100%)',
    borderRadius: '0 0 40px 40px',
  },
  logoArea: {
    position: 'relative',
    zIndex: 1,
    textAlign: 'center',
    paddingTop: 60,
    marginBottom: 32,
  },
  logoIcon: {
    width: 72,
    height: 72,
    borderRadius: 20,
    background: 'rgba(255,255,255,0.2)',
    backdropFilter: 'blur(10px)',
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    margin: '0 auto 16px',
    boxShadow: '0 4px 20px rgba(0,0,0,0.15)',
  },
  appName: {
    fontSize: 28,
    fontWeight: 800,
    color: '#fff',
    letterSpacing: '-0.5px',
  },
  appNameKo: {
    fontSize: 14,
    color: 'rgba(255,255,255,0.7)',
    marginTop: 2,
    fontWeight: 500,
  },
  tagline: {
    fontSize: 13,
    color: 'rgba(255,255,255,0.65)',
    lineHeight: 1.5,
    marginTop: 12,
  },
  loginCard: {
    position: 'relative',
    zIndex: 1,
    background: '#fff',
    borderRadius: 20,
    padding: '28px 24px',
    width: 'calc(100% - 48px)',
    maxWidth: 400,
    boxShadow: '0 8px 32px rgba(0,0,0,0.1)',
    marginBottom: 24,
  },
  loginTitle: {
    fontSize: 20,
    fontWeight: 700,
    color: '#1A1A2E',
    marginBottom: 4,
  },
  loginDesc: {
    fontSize: 13,
    color: '#8888A8',
    marginBottom: 20,
  },
  errorBox: {
    background: '#FEF2F2',
    color: '#DC2626',
    fontSize: 12,
    padding: '10px 14px',
    borderRadius: 10,
    marginBottom: 16,
    display: 'flex',
    alignItems: 'center',
  },
  kakaoBtn: {
    width: '100%',
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    background: '#FEE500',
    color: '#3C1E1E',
    fontSize: 15,
    fontWeight: 600,
    padding: '14px 20px',
    borderRadius: 12,
    border: 'none',
    cursor: 'pointer',
    transition: 'opacity 0.2s',
    marginBottom: 16,
  },
  divider: {
    display: 'flex',
    alignItems: 'center',
    gap: 12,
    marginBottom: 16,
  },
  dividerLine: {
    flex: 1,
    height: 1,
    background: '#EEEEF2',
  },
  dividerText: {
    fontSize: 12,
    color: '#A0A0B8',
    fontWeight: 500,
  },
  demoBtn: {
    width: '100%',
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    background: 'linear-gradient(135deg, #4A6CF7, #7DD3FC)',
    color: '#fff',
    fontSize: 15,
    fontWeight: 600,
    padding: '14px 20px',
    borderRadius: 12,
    border: 'none',
    cursor: 'pointer',
    boxShadow: '0 4px 16px rgba(74,108,247,0.25)',
    transition: 'opacity 0.2s',
    marginBottom: 12,
  },
  demoHint: {
    fontSize: 11,
    color: '#A0A0B8',
    textAlign: 'center',
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
  },
  featuresRow: {
    display: 'flex',
    gap: 16,
    padding: '0 24px',
    marginBottom: 32,
  },
  featureChip: {
    display: 'flex',
    flexDirection: 'column',
    alignItems: 'center',
    background: '#fff',
    borderRadius: 14,
    padding: '14px 16px',
    boxShadow: '0 2px 8px rgba(0,0,0,0.04)',
    flex: 1,
  },
  footer: {
    textAlign: 'center',
    padding: '0 24px 32px',
  },
}
